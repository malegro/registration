function parameter_search(diretorios)

nCasos = length(diretorios);

thresh_IG = 0.1;

load nomes.mat
dados = reamostra_dados(500,diretorios,'early.mat');
classes = dados(:,end);
dados = dados(:,1:end-1);
dados = rescale2(dados,0,1);
dados = cat(2,dados,classes);
rank = calcula_IG(dados,nomes);
chars = rank(find(rank(:,2) >= thresh_IG));
chars = chars' +1;
